...
//begin of functions to inplement automatic player


//move each chess to see the evaluation of each move
//select the top 5 moves
//move the oppnent's chess based on the 5 top moves
//find the most disadvantageous move of each of the 5 top moves and calculate the evaluation
//find the move own chess based on the former moves, evaluate all moves
//select the best in each of the 5 and calculate the sum evaluation
//select the local best move and do it

//calculate the evaluation the given chess
//the computer plays the red chesses the player plays the yellow ones
//if the given chess is a red one, return positive value
//if the given chess is yellow one, return negative value 
//the returning value has been weighed
//input: coordinate,
//output: evaluation
int evaluate_chess(chessboard chessboard_real, int from_cordx, int from_cordy)
{
	int weight_of_computer = 1;
	int weight_of_player = 1;
	int value = 0;
	int i, j;
	if (chessboard_real.what_is_here(from_cordx, from_cordy) == RED_LOCAL)
	{
		for (i = 0; i < 25; i++)
		{
			for (j = 0; j < 17; j++)
			{
				if (chessboard_real.what_is_here(i, j) == RED_LOCAL)
				{
					value += 16 - j;
				}
			}
		}
		return value * weight_of_computer;
	}
	else if (chessboard_real.what_is_here(from_cordx, from_cordy == YELLOW_LOCAL))
	{
		for (i = 0; i < 25; i++)
		{
			for (j = 0; j < 17; j++)
			{
				if (chessboard_real.what_is_here(i, j) == YELLOW_LOCAL)
				{
					value -= j;
				}
			}
		}
		return value * weight_of_player;
	}
}


//select the top five move
//find all the possible moves and select the 5 ones that has the highest evaluation
//input: chessboard
//output: 25-dimenssion vector, first 5 being the evaluation, last 20 being the according fromx, fromy, tox, toy;
int* select_top_five_move(chessboard chessboard_real, int* move_info)
{
	int* move_coord = new int[20]();
	int* move_evalu = new int[5]();
	int* coord_temp = new int[93]();
	int temp_evaluation;
	int i, j, k, n, m;
	int temp = 0;
	for (i = 0; i < 25; i++)
	{
		for (j = 0; j < 17; j++)
		{
			if (chessboard_real.what_is_here(i, j) == RED_LOCAL)
			{
				coord_temp = check_possible_move(i, j, chessboard_real);
				for (k = 1; k < coord_temp[0]; k += 2)
				{
					chessboard_real.update_the_board(i, j, coord_temp[k], coord_temp[k + 1]);
					temp_evaluation = evaluate_chess(chessboard_real, coord_temp[k], coord_temp[k + 1]);
					chessboard_real.update_the_board(coord_temp[k], coord_temp[k + 1], i, j);
					temp = move_evalu[0];
					m = 0;
					for (n = 0; n < 5; n++)
					{
						if (move_evalu[n] <= temp)
						{
							temp = move_evalu[n];
							m = n;
						}
					}
					if (temp <= temp_evaluation)
					{
						move_coord[4 * m] = i;
						move_coord[4 * m + 1] = j;
						move_coord[4 * m + 2] = coord_temp[k];
						move_coord[4 * m + 3] = coord_temp[k + 1];
						move_evalu[m] = temp_evaluation;
					}
				}
			}
		}
	...
	for (n = 0; n < 5; n++)
	{
		move_info[n] = move_evalu[n] * 5;
	}
	for (n = 0; n < 20; n++)
	{
		move_info[n + 5] = move_coord[n];
	}
	delete[] move_coord;
	delete[] move_evalu;
	delete[] coord_temp;
...
//evaluate the next moves and return the final move information
int* evaluate_next(chessboard chessboard_real, int* move_info)
{
	int* coord_temp = new int[93]();
	int* move_opp = new int[4]();
	int* move_own = new int[4]();
	int evaluation = 0;
	int temp_evaluation;
	int i, j, k, p, n;
	for (k = 0; k < 5; k++)
	{
		chessboard_real.update_the_board(move_info[4 * k + 5], move_info[4 * k + 6], move_info[4 * k + 7], move_info[4 * k + 8]);
		for (i = 0; i < 25; i++)
		{
			for (j = 0; j < 17; j++)
			{
				...
				{
					coord_temp = check_possible_move(i, j, chessboard_real);
					for (p = 1; p < coord_temp[0]; p += 2)
					{
						chessboard_real.update_the_board(i, j, coord_temp[p], coord_temp[p + 1]);
						temp_evaluation = evaluate_chess(chessboard_real, coord_temp[p], coord_temp[p + 1]);
						chessboard_real.update_the_board(coord_temp[p], coord_temp[p + 1], i, j);
						if (temp_evaluation < evaluation)
						{
							evaluation = temp_evaluation;
							move_opp[0] = i;
							move_opp[1] = j;
							move_opp[2] = coord_temp[p];
							move_opp[3] = coord_temp[p + 1];
						}
					}
				}
			}
		}
		move_info[k] += evaluation * 1;
		chessboard_real.update_the_board(move_opp[0], move_opp[1], move_opp[2], move_opp[3]);
		for (i = 0; i < 25; i++)
		{
			for (j = 0; j < 17; j++)
			{
				...
				{
					coord_temp = check_possible_move(i, j, chessboard_real);
					for (p = 1; p < coord_temp[0]; p += 1)
					{
						chessboard_real.update_the_board(i, j, coord_temp[p], coord_temp[p + 1]);
						temp_evaluation = evaluate_chess(chessboard_real, coord_temp[p], coord_temp[p + 1]);
						chessboard_real.update_the_board(coord_temp[p], coord_temp[p + 1], i, j);
						if (evaluation < temp_evaluation)
						{
							evaluation = temp_evaluation;
							move_own[0] = i;
							move_own[1] = j;
							move_own[2] = coord_temp[p];
							move_own[3] = coord_temp[p + 1];
						}
					}
				}
			}
		}
		move_info[k] += evaluation * 4;
		chessboard_real.update_the_board(move_opp[2], move_opp[3], move_opp[0], move_opp[1]);
		chessboard_real.update_the_board(move_info[4 * k + 7], move_info[4 * k + 8], move_info[4 * k + 5], move_info[4 * k + 6]);
	}
	delete[] coord_temp;
	delete[] move_opp;
	return move_info;
...
//calculate the auto move coordinates
//using the two upper functions
//input: chessboard, 4-dimensional vector
//output: 4-dimensional vector
int* auto_move(chessboard chessboard_real, int* auto_move_coord)
{
	int* move_info = new int[25]();
	int i, j;
	int temp;
	int n, m;
	move_info = select_top_five_move(chessboard_real, move_info);
	move_info = evaluate_next(chessboard_real, move_info);
	temp = move_info[0];
	j = 0;
	for (i = 0; i < 5; i++)
	{
	...
	auto_move_coord[0] = move_info[4 * j + 5];
	auto_move_coord[1] = move_info[4 * j + 6];
	auto_move_coord[2] = move_info[4 * j + 7];
	auto_move_coord[3] = move_info[4 * j + 8];
	delete[] move_info;
	return auto_move_coord;
}
	...
	while (continue_play)
	{
		...
		while (!chessboard_real.game_over())//进入游戏界面
		{
		 begin:loadchessimg(chessboard_real);
			if (mode_input == 1 && chessboard_real.whos_turn(false, true) == 1)
			{
				auto_move_coord = auto_move(chessboard_real, auto_move_coord);
				chessboard_real.update_the_board(auto_move_coord[0], auto_move_coord[1], auto_move_coord[2], auto_move_coord[3]);
